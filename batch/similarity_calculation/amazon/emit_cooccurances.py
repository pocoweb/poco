...
def getLogger():
    return logging.getLogger("emit_cooccurances")


def output_cooccurance(f, last_user, item_ids):
    if len(item_ids) > 800:
        getLogger().info("One User Skipped: id=%s len(item_ids)=%s" % (last_user, len(item_ids)))
        return
...
def emit_cooccurances(input_path, output_path):
    ...
    for line in open(input_path):
        ...
        count += 1
        if count % 5000 == 0:
            getLogger().debug("progress:%s" % (count / float(6185990) * 100))
